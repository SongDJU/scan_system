...
// 감시 시작
export function startWatching(folder: WatchFolder) {
  ...
  watcher.on('add', (filePath) => {
    ...
    // 이미 처리된 파일인지 확인 (파일명에 '_'가 포함되어 있으면 이미 처리된 것으로 간주)
    // 단, 원본 파일명에 '_'가 있을 수 있으므로 DB에서 확인
    ...
    if (existingProcess) {
      console.log(`이미 처리된 파일 건너뜀: ${filename}`);
      return;
    }
