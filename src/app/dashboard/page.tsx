...
import type { UserSession, FileProcess, WatchFolder, ProcessLog } from '@/types';
...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [statsData, filesData, foldersData, logsData] = await Promise.all([
        statsRes.json(),
        filesRes.json(),
        foldersRes.json(),
        logsRes.json(),
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-6">
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-500">기존</p>
        ...
        {activeTab === 'files' && (
          <>
            ...
            <Card>
              <CardHeader>
                <CardTitle>파일 목록 ({files.length}개)</CardTitle>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    ...
                    {logs.length === 0 ? (
                      ...
                      logs.map((log) => (
                        <TableRow key={log.id}>
                          ...
                          <TableCell>
                            <Badge className={getLogActionColor(log.action)}>
                              {log.action}
                            </Badge>
                          </TableCell>
